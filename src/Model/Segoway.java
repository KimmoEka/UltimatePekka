// TODO: Visual count-down only, no sound? Or some sort of sound interface and container for Sound class (can't implement interface on static methods)?

import lejos.remote.ev3.RemoteBattery;
...
public class Segoway extends Thread{ // TODO: Thread should be a private inner class.
	...
	private static ServerSocket serveri;
	...
	//TAIKA-ARVOT
	private static final double KGYROANGLE = 13; //7.5 aluksi
	private static final double KGYROSPEED = 0.5; //1.15
	private static final double KPOS = 0.1; //0.07
	private static final double KSPEED = 0.08; // 0.1
...
//		}
	...
	private void updateGyroData() {
		...
		//float intersect = (gyroRaw) - ((float) (4.891 * LocalEV3.get().getPower().getVoltage()));
		gOffset = EMAOFFSET * gyroRaw + (1-EMAOFFSET) * gOffset;
		gyroSpeed = gyroRaw - gOffset; // Angular velocity (degrees/sec)
		//gyroSpeed = gyroRaw - intersect - ((float) (4.891 * LocalEV3.get().getPower().getVoltage()));
	...
	public void run() {
		...
		while(true) {
			...
			//System.out.println("s" + gyroAngle);
...
//			try {
//				out.writeUTF(" gyroSpeed: " + gyroSpeed + " gyroAngle: " + gyroAngle + " motorpos: " 
//			+ motorPos + " motorSpeed: " + motorSpeed + " motorCont: " + motorControlDrive);
//				out.flush();
//				out.writeUTF("p " + power);
//				out.flush();
//			} catch (IOException e) {
